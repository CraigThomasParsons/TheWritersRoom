...
@section('header')
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Stories</h1>
...
@section('content')
    ...
    <div class="mb-6 bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        <form method="GET" action="{{ route('stories.index') }}" class="flex flex-wrap gap-4 items-end">
            ...
            <div class="w-48">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Epic</label>
                <select name="epic_id" 
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <a href="{{ route('stories.index') }}" class="px-4 py-2 text-gray-600 dark:text-gray-400 hover:text-gray-800">
                Clear
            </a>
        </form>
    </div>
        ...
        <ul class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                <a href="{{ route('stories.show', $story) }}" 
                                   class="text-base font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800">
                                    {{ $story->title }}
                                </a>
                                @if ($story->priority > 0)
                                    <span class="text-xs text-orange-600 font-medium">P{{ $story->priority }}</span>
                                @endif
                                @if ($story->est_points)
            ...
            @empty
                <li class="px-6 py-12 text-center">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No stories</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by creating a new story.</p>
                    <div class="mt-6">
                        <a href="{{ route('stories.create') }}" 
