...
@section('header')
    <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Create Epic</h1>
@endsection

@section('content')
    <div class="max-w-2xl">
        <form method="POST" action="{{ route('epics.store') }}" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="summary" id="summary" rows="4"
                              ...
                              class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('summary') }}</textarea>
                    @error('summary')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="epic_status_id" id="epic_status_id" required
                        ...
                        @foreach ($statuses as $status)
                            <option value="{{ $status->id }}" {{ old('epic_status_id', $statuses->firstWhere('key', 'backlog')?->id) == $status->id ? 'selected' : '' }}>
                                {{ $status->name }}
                            </option>
                        @endforeach
                    </select>
